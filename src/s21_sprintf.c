...
void sprint_d (flags* flags, int* counter, int flag_zero, char** str, _string* data) {
    if (flags->zero == 1 && flags->align == 1)
        flags->zero = 0;
...
void read_flags (flags* flags, const char** format) {
    if ((**format >= '1' && **format <= '9') || (flags->dot && **format >= '0' && **format <= '9')) {
        get_int_from_char(format, flags);
//        printf("acc = %d   ", flags->acc);
//        printf("w = %d\n", flags->width);
        
    }
    else if (**(format) == '-') flags->align = 1;
    else if (**(format) == '+') flags->sign = 1;
    else if (**(format) == ' ') flags->space = 1;
    else if (**(format) == '.') flags->dot = 1;
    else if (**(format) == '0') flags->zero = 1;
    else if (**(format) == 'l') flags->l = 1;
    else if (**(format) == 'h') flags->h = 1;
    else if (**(format) == 'L') flags->L = 1;
...
//        printf("i = %d  %c", i, data->buffer[i]);
    ...
    if (flags->zero &&(flags->sign || zero_flag < 0 || flags->space) && ((flags->acc < 0 && !flag_d_f) || flag_d_f)) {
        --data->pos;
        --flags->width;
    }
    ...
    if (flags->zero &&flags->sign &&((flags->acc < 0 && !flag_d_f) || flag_d_f)&& zero_flag >= 0)
        data->buffer[data->pos++] = '+';
    else if (flags->zero&&((flags->acc < 0 && !flag_d_f) || flag_d_f)&& zero_flag < 0)
        data->buffer[data->pos++] = '-';
    else if (flags->zero&&((flags->acc < 0 && !flag_d_f) || flag_d_f)&& flags->space)
        data->buffer[data->pos++] = ' ';
    if (!flags->align) shuffle_str(data);
//    for (int i = 0; i < data->pos; ++i)
